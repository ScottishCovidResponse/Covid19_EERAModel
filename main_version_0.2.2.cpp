...
void read_parameters(int& herd_id, double& tau, int&num_threads,int& nsteps, int& nParticLimit, int& nSim, double& kernelFactor, vector<double>& toleranceLimit,
		double& T_lat, double& p_s, double& juvp_s ,double& T_in,double& T_rec, double& T_sym ,double& T_hos ,int& day_shut, int& totN_hcw,
...
void select_obs(int& Npop, int& t_index, int& duration, int& day_intro, int& day_shut, 
	vector<int>& obsHosp_tmp, vector<vector<int> > data_tmp, int herd_id, 
	int T_inf, int T_sym);

	//fitting process functions
void model_select(int smc,particle &outvec, vector<vector<double>> fixed_parameters,vector<vector<double>> cfr_byage,
	vector<vector<double>> waifw_norm, vector<vector<double>> waifw_sdist, vector<vector<double>> waifw_home, 
	vector <int> agenums, double tau, int duration, int day_intro, int day_shut, int Npop, gsl_rng * r);
...
double sse_calc_int(vector<int> simHosp);
...
double sst_calc_int(int Npop, vector<int> simHosp);
	...
	//model functions
void my_model(vector<double> parameter_set, vector<vector<double>> fixed_parameters, vector<vector<double>> cfr_byage,
				vector<vector<double>> waifw_norm, vector<vector<double>> waifw_sdist, vector<vector<double>> waifw_home,
				int duration, int day_intro, int day_shut, int Npop, vector<int> agenums,int dfseed, 
...
int main(int argc, char **argv) {
	...
	double T_lat, p_s ,juvp_s, T_inf, T_rec, T_sym ,T_hos;
	...
	read_parameters(herd_id, tau, num_threads, nsteps, nParticLimit,nSim,kernelFactor,toleranceLimit,
			T_lat, p_s ,juvp_s, T_inf, T_rec, T_sym ,T_hos,day_shut, totN_hcw,
	...
	vector<vector<double>> fixed_parameters(waifw_norm.size());
	vector<double> paramlist = {T_lat, p_s ,T_inf, T_rec, T_sym ,T_hos};
	
	for (unsigned int var = 0; var < fixed_parameters.size(); ++var) {
		fixed_parameters[var] = paramlist;
	}
	fixed_parameters[0][1] = juvp_s;

	//Separate case information for each herd_id
	vector<int> obsHosp_tmp;
	int Npop=0;
	int t_index=-1;
	...
	select_obs(	Npop, t_index, duration, day_intro, day_shut, 
		obsHosp_tmp, data_tmp, herd_id, T_inf, T_sym);
	obsHosp = obsHosp_tmp;
	obsHosp_tmp.clear();	
	...
	for (smc = 0; smc < nsteps; ++smc) {//todo:
		...
		output_step << "iterID" << "," << "nsse" << "," << "p_inf" << "," << "p_hcw" << "," << "c_hcw" << "," << "d" << "," << "q"  << "," << "weight\n";
...
//				outs_vec.sum_sq = 1.e06;
				...
				model_select(smc,outs_vec, fixed_parameters, cfr_byage,waifw_norm,waifw_sdist,waifw_home,agenums,
								tau, duration, day_intro, day_shut, Npop, r);
				...
				if(counter < nParticLimit && outs_vec.nsse <= toleranceLimit[smc]){
					//#pragma omp critical
					{
		...
		for (kk = 0; kk < Nparticle; ++kk) {
			output_step << particleList1[kk].iter << ", " << particleList1[kk].nsse <<  ", " ;
...
double sse_calc_int(vector<int> simHosp){
	...
	for (unsigned int xx = 0; xx < obsHosp.size(); ++xx) {
		double error = 0.0;
		error = (double)obsHosp[xx] - (double)simHosp[xx] ;
		sum_sq += pow(error,2);
	}
...
void model_select(int smc,particle &outvec, vector<vector<double>> fixed_parameters,vector<vector<double>> cfr_byage,
	...
	//main loop
	my_model(outvec.parameter_set, fixed_parameters, cfr_byage,waifw_norm, waifw_sdist, waifw_home,
					duration, day_intro, day_shut, Npop,agenums,dfseed, tau, r, sim_status,ends,
					death_status,deathH_status);

	//---------------------------------------
	// compute the  sum of squared errors
	//---------------------------------------
	double sum_sq = 1000000.0,nsse=1000000.0;
	sum_sq= sse_calc_int(sim_status);
	...
	nsse = sqrt(sum_sq)/ (double)max_obs;
	
	
//	cout << "nsse: " << nsse << " , sum_sq: " << sum_sq <<'\n';


/*	//---------------------------------------
	// compute the sum of squared total
	//---------------------------------------
	double sum_sq_tot=1000000.0;
	sum_sq_tot= sst_calc_int(Npop, simHosp);

	//---------------------------------------
	// coefficient of determination -- Rsq
	//---------------------------------------
	double rsq=0.0;
	rsq= 1.0 - sum_sq / sum_sq_tot;
*/
	//---------------------------------------
...
void select_obs(int& Npop, int& t_index, int& duration, int& day_intro, int& day_shut, 
	...
	int intro = (t_index+1)-(T_inf+T_sym);
	...
	if(duration > obsHosp_tmp.size()){
		...
		vector<int> extra_tmp;
		for (int extra = 0; extra < extra_time; ++extra) {
			extra_tmp.push_back(0);
		}		
		for (unsigned int iv = 0; iv < obsHosp_tmp.size(); ++iv) {
			extra_tmp.push_back(obsHosp_tmp[iv]);
		}		
		day_shut = day_shut+ extra_time;
		obsHosp_tmp = extra_tmp;
		
		extra_tmp.clear();	
	}
	
	cout << obsHosp_tmp.size()<<'\n';
...
void infspread(gsl_rng * r, vector<int>& pop, int& deaths, int& deathsH, int& detected, vector<double> parameter_set, 
	vector<double> cfr_tab, double lambda){
    unsigned int S=pop[0], E=pop[1], E_t=pop[2], I=pop[3], I_t=pop[4], I_s=pop[5], H=pop[6], R=pop[7], D=pop[8] ;
	
	double T_lat= parameter_set[0];
	double p_s= parameter_set[1];
	double T_inf= parameter_set[2];
	double T_rec= parameter_set[3];
	double T_sym= parameter_set[4];
	double T_hos= parameter_set[5];
    ...
    // symptomatic
    unsigned int hospitalize = gsl_ran_poisson(r, p_h * (1 / T_sym) * (double)I_s); //symptomatic become hospitalized
    hospitalize = min(I_s, hospitalize);
	H += hospitalize;
	I_s -= hospitalize;
	
    unsigned int newdeathsC = gsl_ran_poisson(r, ( 1 - p_h ) * p_dc * ( 1 / (T_hos / 2)) * (double)I_s); //symptomatic die at home
    newdeathsC = min(I_s, newdeathsC);
    D += newdeathsC;
	I_s -= newdeathsC;
	
    unsigned int recoverI_s = gsl_ran_poisson(r, ( 1 - p_h ) * (1 - p_dc) * ( 1 / T_rec) * (double)I_s); //symptomatic recover at home
...
void my_model(vector<double> parameter_set, vector<vector<double>> fixed_parameters, vector<vector<double>> cfr_byage,
				vector<vector<double>> waifw_norm, vector<vector<double>> waifw_sdist, vector<vector<double>> waifw_home,
				int duration, int day_intro, int day_shut, int Npop,
				vector<int> agenums,int dfseed, double tau, gsl_rng * r, vector<int> &sim_status, vector<vector<int>> &ends,
				vector<int> &death_status,vector<int> &deathH_status){
	...
	for ( int age = 0; age < (n_agegroup); ++age) {
		...
		if(age >=3 & age < (n_agegroup-1) ){
			seed_pop[age-3] = (double)agenums[age];
		}
	}

	//introduce disease at t=0
	int startdz = dfseed; //how many diseased to start with
	...
	for ( int age = 3; age < (n_agegroup-1); ++age) {
		poparray[age][0] -=  startdist[age-3];//take diseased out of S
		poparray[age][3] +=  startdist[age-3];//put diseased in I	
	}
	...
	for (int tt = 1; tt < (int)ceil(duration/tau); ++tt) {
		...
		for ( int age = 0; age < (n_agegroup); ++age) {	
			infspread(r, poparray[age], deaths, deathsH, detected,fixed_parameters[age],cfr_byage[age], lambda[age]);
		}
...
void read_parameters(int& herd_id, double& tau, int&num_threads,int& nsteps, int& nParticLimit, int& nSim, double& kernelFactor, vector<double>& toleranceLimit,
		double& T_lat, double& p_s , double& juvp_s, double& T_inf,double& T_rec, double& T_sym ,double& T_hos , int& day_shut, int& totN_hcw,
		int& nPar, double& prior_pinf_shape1,double& prior_pinf_shape2,double& prior_phcw_shape1,double& prior_phcw_shape2,
		double& prior_chcw_mean,double& prior_d_shape1,double& prior_d_shape2,double& prior_q_shape1, double& prior_q_shape2){
	...
	T_lat = atof(parameters.GetValue("T_lat", "Fixed parameters", parameterfile).c_str());
	p_s = atof(parameters.GetValue("p_s", "Fixed parameters", parameterfile).c_str());
	juvp_s = atof(parameters.GetValue("juvp_s", "Fixed parameters", parameterfile).c_str());
	T_inf = atof(parameters.GetValue("T_inf", "Fixed parameters", parameterfile).c_str());
	T_rec = atof(parameters.GetValue("T_rec", "Fixed parameters", parameterfile).c_str());
	T_sym = atof(parameters.GetValue("T_sym", "Fixed parameters", parameterfile).c_str());
	T_hos = atof(parameters.GetValue("T_hos", "Fixed parameters", parameterfile).c_str());

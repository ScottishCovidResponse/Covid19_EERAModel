...
public:
   ...
   */
  virtual Status Run(std::vector<double> parameter_set, const seed &seedlist,
                     int day_shut, int n_sim_steps) = 0;

  /**
   * @brief Set the fixed parameters in the model
   *
   * @note This interface is required by the prediction framework, as when
   * running in prediction mode, the fixed parameters come from a differen
   * source than when running in inference mode. When this function is called,
   * it will overwrite the fixed parameters that were set when the model object
   * was created.
   *
 ...
 *
 * Provides constants associated with the use of model parameter sets. Model
 * parameter sets are stored as vectors, where each element of the vector
 * corresponds to a particular parameter. In order to avoid the use of lots of
 * magic numbers in the code, this class provides index constants with
 * meaningful names.
 *
 * @note Model parameters are distinct from "fixed parameters" to the model; the
 * latter are also used to configure the behaviour of the model, but are not
 * varies and inferred as part of the inference framework. The former are also
 * referred to as "Inference Parameters" when speaking in the context of the
 * inference framework.
 */
class ModelParameters {
public:
 ...
 * passing from one compartment to another. Also compares current value prior to
 * flow with that after to ensure flow is positive.
 *
 * @param pops_from_val Number of individuals in starting compartment within
 * prior compartments struct
 * @param pops_new_from_val Number of individuals in starting compartment within
 * the compartments struct currently being modified
 * @param rate The rate of flow
 *
 * @return Number of individuals moving/flowing from one compartment to the
 * other
 */
int Flow(Random::RNGInterface::Sptr rng, int pops_from_val,
         int pops_new_from_val, double rate);
 ...
 *
 * Within the input cases data, the total population of the region is located in
 * the first column of the row corresponding to the region
 *
 ...
 *
 * Calculates the number of HCW in the region, assuming that the ratio of the
 * region's population to that of the country is same as that for HCWs only
 *
 * @param regionalPopulation Population of the region
 * @param totalHCW Total number of HCWs in the country
 * @param obs Input observations
 */
int ComputeNumberOfHCWInRegion(int regionalPopulation, int totalHCW,
                               const ObservationsForModels &obs);
 ...
 * @return agenums
 */
std::vector<int> ComputeAgeNums(int shb_id, int Npop, int N_hcw,
                                const ObservationsForModels &obs);
 ...
 *
 * @param cmps_vec Vector of compartments struct containing population per
 * category
 *
 * @return Vector of population counters
 */
std::vector<std::vector<int>>
compartments_to_vector(const std::vector<Compartments> &cmps_vec);

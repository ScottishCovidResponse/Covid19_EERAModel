 ...
 *
 * @param configDir Directory containing the configuration and data files
 * @param index Index of the parameter set to select from the posterior
 * parameters file
 * @param log Logger
 *
 ...
 */
std::vector<double>
ReadPredictionParametersFromFile(const std::string &filePath,
                                 int set_selection);
 ...
 * @param outDirPath Path to the directory in which the output files should be
 * placed
 */
 ...
 * @param end_comps Matrix to hold the end states of the simulation organised by
 * compartments
 ...
 * @param SettingCategory Name of category in which the value is located in the
 * INI file
...
ParseVariableType ReadNumberFromFile(std::string SettingName,
                                     std::string SettingCategory,
                                     const std::string &filePath) {
  if (!Utilities::fileExists(filePath)) {
    throw IOException(filePath + ": File not found!");
  }

  std::string SettingValue =
      CIniFile::GetValue(SettingName, SettingCategory, filePath);
  if (SettingValue.empty()) {
    throw IOException("Variable: " + SettingName + " in Category: " +
                      SettingCategory + " not found in File: " + filePath);
  }

  char *endptr = nullptr;
  ParseVariableType Value;
  if (std::is_same<ParseVariableType, double>::value) {
    Value = strtod(SettingValue.c_str(), &endptr);
  }
  if (std::is_same<ParseVariableType, int>::value) {
    Value = strtol(SettingValue.c_str(), &endptr, 0);
  }

  /* Error Handling for strtod or strtol functions*/
  if (Value == -HUGE_VAL || Value == HUGE_VAL ||
      endptr == SettingValue.c_str()) {
    std::stringstream SettingParseError;
    SettingParseError << std::endl;
    SettingParseError << "Invalid value in Parameter File: " << filePath.c_str()
                      << std::endl;

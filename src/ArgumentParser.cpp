...
ArgumentParser::ArgumentParser(int argc, const char *const *argv) {
  ...
  try {
    TCLAP::CmdLine cmd(
        "Run model with specified set of options", // Help string message
    ...
    TCLAP::ValueArg<std::string> structureArg(
        "s", "structure", "Model structure. Can be original, irish or irish2",
        true, "", &allowedStructures);
    TCLAP::ValueArg<int> indexArg(
        "i", "index",
        "Parameter set index for forward prediction mode. Defaults to zero.",
        false, 0, "integer");
    TCLAP::ValueArg<std::string> modeArg(
        "m", "mode", "Running mode. Can be either inference or prediction.",
        true, "", &allowedModes);
    TCLAP::ValueArg<std::string> dataLocArg(
        "l", "local", "Location of local data repository", false, "", "string");
    TCLAP::ValueArg<std::string> outputDirArg(
        "d", "outdir", "Output directory of data files", false,
        _args.output_dir, "string");
    cmd.add(modeArg);
    cmd.add(dataLocArg);
    ...
    _args.output_dir = (Utilities::directoryExists(outputDirArg.getValue()))
                           ? outputDirArg.getValue()
                           : _args.output_dir;
    ...
    _args.local_location = (!dataLocArg.getValue().empty())
                               ? dataLocArg.getValue()
                               : std::string(ROOT_DIR);
...
void ArgumentParser::logArguments(Utilities::logging_stream::Sptr log) {
  ...
  (*log) << "\t"
         << "Command line: ";
  for (const auto &s : raw_args_)
    (*log) << s << " ";
  (*log) << std::endl;

  (*log) << "\t"
         << "Local: " << ((_args.isLocal) ? "True" : "False") << std::endl;
  if (_args.isLocal) {
    (*log) << "\t"
           << "Local Directory: " << _args.local_location << std::endl;
  }
  (*log) << "\t"
         << "Structure: ";
  if (_args.structure != ModelStructureId(0)) {
    (*log) << ((_args.structure == ModelStructureId::IRISH) ? "Irish"
                                                            : "Original")
           << std::endl;
  } else {
  ...
  if (_args.mode != ModelModeId(0)) {
    (*log) << "\t"
           << "Mode: "
           << ((_args.mode == ModelModeId::INFERENCE) ? "Inference"
                                                      : "Prediction")
           << std::endl;
  } else {
    (*log) << "Default" << std::endl;
  ...
  (*log) << "\t"
         << "Output Directory: " << _args.output_dir << std::endl;
  (*log) << "\t"
         << "Forward prediction parameter set index: "
         << _args.parameter_set_index << std::endl;
}

 ...
 *
 * Summate values at every Nth index, e.g. n = 7 would summate daily data to be
 * weekly data. For cases where n is not a multiple of the vector size, the
 * residual values at the end of the input are dropped.
 *
 * If n is greater than the size of @p data, or the size of @p data is zero, an
 * empty vector is returned.
 *
 * @f$f([0,1,2,3,4,5,6,7], 3) -> f([0,1,2,3,4,5], 4) -> [3,12] @f$
 *
 * @param data	Vector of data to be reduced
 * @param n	Nth value at which summation should occur (i.e. index+1) where
 * @code{.cpp} n <= data.size() @endcode
 *
 ...
 */
template <typename T>
std::vector<std::vector<T>> read_csv(const std::string &inputfile,
                                     char delimiter, bool header = false) {
  std::vector<std::vector<T>> data;
  std::ifstream infile(inputfile.c_str());
  if (infile.fail()) {
    std::cout << "Input file: " << inputfile.c_str() << " not found"
              << std::endl;
    return data;
  }
  std::string line;
    ...
    while (line[linepos] != 0 && linepos < linemax) {
      ...
      if (!inquotes && curstring.length() == 0 && c == '"') { // beginquotechar
        ...
        if ((linepos + 1 < linemax) && (line[linepos + 1] == '"')) {
          curstring.push_back(c); // encountered 2 double quotes in a row
                                  // (resolves to 1 double quote)
          linepos++;
        } else {
          inquotes = false;
        }                                       // endquotechar

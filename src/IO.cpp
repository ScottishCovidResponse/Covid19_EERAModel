...
void ImportConsistencyCheck(const std::string &filePath,
                            const unsigned int &axisLength,
...
ValidationParameters ImportValidationParameters(const std::string &configDir) {
  ...
  parameters.nHealthBoards =
      ReadNumberFromFile<int>("nHealthBoards", "Settings", filePath);
  parameters.nAgeGroups =
      ReadNumberFromFile<int>("nAgeGroups", "Settings", filePath);
  parameters.nCfrCategories =
      ReadNumberFromFile<int>("nCfrCategories", "Settings", filePath);
  parameters.nCasesDays =
      ReadNumberFromFile<int>("nCasesDays", "Settings", filePath);

  return parameters;
}

seed ReadSeedSettings(const std::string &ParamsPath,
                      Utilities::logging_stream::Sptr log) {
  seed seedSettings;

  std::string SettingName("seedmethod");
  std::string SettingsCategory("Seed settings");
  seedSettings.seedmethod =
      CIniFile::GetValue(SettingName, SettingsCategory, ParamsPath);
  if (seedSettings.seedmethod == "background") {
    seedSettings.hrp =
        ReadNumberFromFile<int>("hrp", SettingsCategory, ParamsPath);
  } else {
    if (seedSettings.seedmethod != "random") {
      (*log) << "WARNING: unknown seeding method, defaulting to random"
             << std::endl;
    }

    seedSettings.nseed =
        ReadNumberFromFile<int>("nseed", SettingsCategory, ParamsPath);
  }

  seedSettings.use_fixed_seed = static_cast<bool>(
      ReadNumberFromFile<int>("use_fixed_seed", SettingsCategory, ParamsPath));
  seedSettings.seed_value =
      ReadNumberFromFile<int>("seed_value", SettingsCategory, ParamsPath);
...
params ReadFixedModelParameters(const std::string &ParamsPath) {
  ...
  paramlist.T_lat =
      ReadNumberFromFile<double>("T_lat", SettingsCategory, ParamsPath);
  paramlist.juvp_s =
      ReadNumberFromFile<double>("juvp_s", SettingsCategory, ParamsPath);
  paramlist.T_inf =
      ReadNumberFromFile<double>("T_inf", SettingsCategory, ParamsPath);
  paramlist.T_rec =
      ReadNumberFromFile<double>("T_rec", SettingsCategory, ParamsPath);
  paramlist.T_sym =
      ReadNumberFromFile<double>("T_sym", SettingsCategory, ParamsPath);
  paramlist.T_hos =
      ReadNumberFromFile<double>("T_hos", SettingsCategory, ParamsPath);
  paramlist.K = ReadNumberFromFile<int>("K", SettingsCategory, ParamsPath);
  paramlist.inf_asym =
      ReadNumberFromFile<double>("inf_asym", SettingsCategory, ParamsPath);

  return paramlist;
}

SupplementaryInputParameters
ReadSupplementaryParameters(const std::string &ParamsPath,
                            Utilities::logging_stream::Sptr log) {
...
CommonModelInputParameters ReadCommonParameters(const std::string &ParamsPath) {
  ...
  commonParameters.herd_id =
      ReadNumberFromFile<int>("shb_id", "Settings", ParamsPath);
  commonParameters.totN_hcw =
      ReadNumberFromFile<int>("totN_hcw", "Fixed parameters", ParamsPath);

  return commonParameters;
}

InferenceConfig ReadInferenceConfig(const std::string &configDir,
                                    Utilities::logging_stream::Sptr log) {
  ...
  inferenceConfig.herd_id =
      ReadNumberFromFile<int>("shb_id", "Settings", ParamsPath);
  inferenceConfig.day_shut =
      ReadNumberFromFile<int>("day_shut", "Fixed parameters", ParamsPath);
  inferenceConfig.tau =
      ReadNumberFromFile<double>("tau", "Settings", ParamsPath);

  inferenceConfig.prior_pinf_shape1 = ReadNumberFromFile<double>(
      "prior_pinf_shape1", "Priors settings", ParamsPath);
  inferenceConfig.prior_pinf_shape2 = ReadNumberFromFile<double>(
      "prior_pinf_shape2", "Priors settings", ParamsPath);
  inferenceConfig.prior_phcw_shape1 = ReadNumberFromFile<double>(
      "prior_phcw_shape1", "Priors settings", ParamsPath);
  inferenceConfig.prior_phcw_shape2 = ReadNumberFromFile<double>(
      "prior_phcw_shape2", "Priors settings", ParamsPath);
  inferenceConfig.prior_chcw_mean = ReadNumberFromFile<double>(
      "prior_chcw_mean", "Priors settings", ParamsPath);
  inferenceConfig.prior_d_shape1 = ReadNumberFromFile<double>(
      "prior_d_shape1", "Priors settings", ParamsPath);
  inferenceConfig.prior_d_shape2 = ReadNumberFromFile<double>(
      "prior_d_shape2", "Priors settings", ParamsPath);
  inferenceConfig.prior_q_shape1 = ReadNumberFromFile<double>(
      "prior_q_shape1", "Priors settings", ParamsPath);
  inferenceConfig.prior_q_shape2 = ReadNumberFromFile<double>(
      "prior_q_shape2", "Priors settings", ParamsPath);
  inferenceConfig.prior_lambda_shape1 = ReadNumberFromFile<double>(
      "prior_lambda_shape1", "Priors settings", ParamsPath);
  inferenceConfig.prior_lambda_shape2 = ReadNumberFromFile<double>(
      "prior_lambda_shape2", "Priors settings", ParamsPath);

  inferenceConfig.prior_ps_shape1 = ReadNumberFromFile<double>(
      "prior_ps_shape1", "Priors settings", ParamsPath);
  inferenceConfig.prior_ps_shape2 = ReadNumberFromFile<double>(
      "prior_ps_shape2", "Priors settings", ParamsPath);
  inferenceConfig.prior_rrd_shape1 = ReadNumberFromFile<double>(
      "prior_rrd_shape1", "Priors settings", ParamsPath);
  inferenceConfig.prior_rrd_shape2 = ReadNumberFromFile<double>(
      "prior_rrd_shape2", "Priors settings", ParamsPath);

  inferenceConfig.nsteps =
      ReadNumberFromFile<int>("nsteps", "Fit settings", ParamsPath);
  inferenceConfig.kernelFactor =
      ReadNumberFromFile<double>("kernelFactor", "Fit settings", ParamsPath);
  inferenceConfig.nSim =
      ReadNumberFromFile<int>("nSim", "Fit settings", ParamsPath);
  inferenceConfig.nParticleLimit =
      ReadNumberFromFile<int>("nParticLimit", "Fit settings", ParamsPath);
  ...
  for (int ii = 0; ii < inferenceConfig.nsteps; ii++) {
    ...
    KeyName << "Key" << (ii + 1);
    inferenceConfig.toleranceLimit[ii] = ReadNumberFromFile<double>(
        KeyName.str(), "Tolerance settings", ParamsPath);
  }
...
PredictionConfig ReadPredictionConfig(const std::string &configDir, int index,
  ...
  predictionConfig.day_shut =
      ReadNumberFromFile<int>("day_shut", "Fixed parameters", filePath);

  std::string sectionId("Prediction Configuration");
  predictionConfig.n_sim_steps =
      ReadNumberFromFile<int>("n_sim_steps", sectionId, filePath);

  predictionConfig.index = index;

  std::string parametersFile(configDir + "/posterior_parameters.csv");

  // The posterior parameters are columns 0 to 7; the fixed parameters are
  // columns 8 to 15
  std::vector<double> modelParameters =
      ReadPredictionParametersFromFile(parametersFile, predictionConfig.index);
  predictionConfig.posterior_parameters =
      std::vector<double>(modelParameters.begin(), modelParameters.begin() + 8);
...
ObservationsForInference
ReadInferenceObservations(const std::string &configDir,
                          Utilities::logging_stream::Sptr log) {
  ...
  // Note: first vector is the vector of time. value of -1 indicate number of
  // pigs in the herd rows from 1 are indivudual health board last row is for all
  // of scotland
  ...
  ImportConsistencyCheck(scot_data_file, cases_rows, (nHealthBoards + 1),
                         "rows");
  ...
  ImportConsistencyCheck(scot_deaths_file, deaths_rows, (nHealthBoards + 1),
                         "rows");
...
ObservationsForModels
ReadModelObservations(const std::string &configDir,
                      Utilities::logging_stream::Sptr log) {
  ...
  ValidationParameters validationParameters =
      ImportValidationParameters(configDir);
  ...
  // rows are for each age group: [0] Under20,[1] 20-29,[2] 30-39,[3] 40-49,[4]
  // 50-59,[5] 60-69,[6] Over70,[7] HCW
  ...
  ImportConsistencyCheck(scot_ages_file, age_pop_cols, (nAgeGroups - 1),
                         "columns");
  ...
  ImportConsistencyCheck(waifw_norm_file, waifw_norm_cols, nAgeGroups,
                         "columns");
  ...
  ImportConsistencyCheck(waifw_home_file, waifw_home_cols, nAgeGroups,
                         "columns");
  ...
  ImportConsistencyCheck(waifw_sdist_file, waifw_sdist_rows, nAgeGroups,
                         "rows");
  ImportConsistencyCheck(waifw_sdist_file, waifw_sdist_cols, nAgeGroups,
                         "columns");
  ...
  // columns are for each age group: [0] Under20,[1] 20-29,[2] 30-39,[3]
  // 40-49,[4] 50-59,[5] 60-69,[6] Over70,[7] HCW rows are for each individual
  // Health Borad last row is for Scotland
...
std::vector<double> ReadPosteriorParametersFromFile(const std::string &filePath,
                                                    int set_selection) {
  ...
  if (set_selection >= static_cast<int>(lines.size())) {
    ...
    SetSelectError
        << "Parameter set selection out of bounds! Please select between 0-"
        << (lines.size() - 1) << "..." << std::endl;
  ...
  if ((last - first) != nPar) {
    ...
    PosteriorFileFormatError
        << "Please check formatting of posterior parameter input file, 8 "
...
std::vector<double>
ReadPredictionParametersFromFile(const std::string &filePath, int index) {
  // Temporary matrix to hold data from input file
  std::vector<std::vector<double>> lines;
  char delimiter = ',';
  ...
  // There are 16 columns in the file: 8 posterior parameters + 8 fixed
  // parameters
  const int nPar = 16;
  if ((last - first) != nPar) {
    std::stringstream PosteriorFileFormatError;
    PosteriorFileFormatError
        << "Please check formatting of posterior parameter input file, 16 "
           "parameter values are needed..."
        << std::endl;
    throw std::runtime_error(PosteriorFileFormatError.str());
  }
...
void WriteOutputsToFiles(int smc, int herd_id, int Nparticle, int nPar,
  ...
  namefile << (outDirPath + "/output_abc-smc_particles_step") << smc << "_shb"
           << herd_id << "_" << log->getLoggerTime() << ".txt";
  namefile_simu << (outDirPath + "/output_abc-smc_simu_step") << smc << "_shb"
                << herd_id << "_" << log->getLoggerTime() << ".txt";
  namefile_ends << (outDirPath + "/output_abc-smc_ends_step") << smc << "_shb"
                << herd_id << "_" << log->getLoggerTime() << ".txt";
  ...
  output_step << "iterID,nsse_cases,nsse_deaths,p_inf,p_hcw,c_hcw,d,q,p_s,rrd,"
                 "intro,weight"
              << std::endl;
  ...
  // add the column names for each output list of the compartment values of the
  // last day of the chosen simulations
  output_ends << "iterID"
              << ","
              << "age_group"
              << ","
  ...
  // outputs the list of particles (and corresponding predictions) that were
  // accepted at each steps of ABC-smc
  for (int kk = 0; kk < Nparticle; ++kk) {
    output_step << particleList[kk].iter << ", " << particleList[kk].nsse_cases
                << ", " << particleList[kk].nsse_deaths << ", ";
    ...
    for (unsigned int var = 0; var < particleList[kk].simu_outs.size(); ++var) {
      output_simu << particleList[kk].iter << ", " << var << ", "
                  << particleList[kk].simu_outs[var] << ", "
                  << particleList[kk].hospital_death_outs[var] << ", "
                  << particleList[kk].death_outs[var] << '\n';
    }

    for (unsigned int age = 0; age < particleList[kk].end_comps.size(); ++age) {
      for (unsigned int var = 0; var < particleList[kk].end_comps[0].size();
           ++var) {
        output_ends << particleList[kk].iter << ", " << age << ", " << var
                    << ", " << particleList[kk].end_comps[age][var] << '\n';
      }
    }
  }
...
void WritePredictionsToFiles(Status status,
                             std::vector<std::vector<int>> &end_comps,
                             const std::string &outDirPath,
                             const Utilities::logging_stream::Sptr &log) {
  std::stringstream namefile_simu, namefile_ends, namefile_full;
  namefile_simu << (outDirPath + "/output_prediction_simu") << "_"
                << log->getLoggerTime() << ".txt";
  namefile_ends << (outDirPath + "/output_prediction_ends") << "_"
                << log->getLoggerTime() << ".txt";
  namefile_full << (outDirPath + "/output_prediction_full") << "_"
                << log->getLoggerTime() << ".txt";
  ...
  for (unsigned int var = 0; var < status.pop_array.size(); ++var) {
    std::vector<std::vector<int>> pop_array_compartment_to_vector =
        Model::compartments_to_vector(status.pop_array[var]);
    ...
    if (var < status.pop_array.size() - 1) {
      output_full << '\n';
    }
  }
...
void LogFixedParameters(const params &paramlist,
                        Utilities::logging_stream::Sptr log) {
  (*log) << "[Fixed parameter values]:\n";
  OutputFixedParameters(log, paramlist);
}

void LogRandomiserSettings(const SupplementaryInputParameters &params,
                           unsigned long randomiser_seed,
                           Utilities::logging_stream::Sptr log) {
  (*log) << "[Randomisation Settings]:\n";
  (*log) << "    Seed type: ";
  (*log) << ((params.seedlist.use_fixed_seed) ? "Fixed" : "Time based")
         << std::endl;
...
void LogSeedSettings(const seed &params, Utilities::logging_stream::Sptr log) {
  ...
  if (params.seedmethod == "random") {
    ...
    (*log) << "    duration of the high risk period (hrp): " << params.hrp
           << std::endl;
  }
}

void LogPredictionConfig(const PredictionConfig &config,
                         Utilities::logging_stream::Sptr log) {
...
void LogGitVersionInfo(Utilities::logging_stream::Sptr log) {
  ...
  (*log) << "    Uncommitted changes: "
         << (GitMetadata::AnyUncommittedChanges() ? "Yes" : "No") << std::endl;
}

void LogDependencyVersionInfo(Utilities::logging_stream::Sptr log) {
  (*log) << "[Dependency Versioning]" << std::endl;
  (*log) << "    Compiler id: " << DependencyVersions::CompilerId()
         << std::endl;
  (*log) << "    Compiler version: " << DependencyVersions::CompilerVersion()
         << std::endl;
  (*log) << "    CMake version: " << DependencyVersions::CMakeVersion()
         << std::endl;
  (*log) << "    GSL version: " << DependencyVersions::GSLVersion()
         << std::endl;
}

void OutputFixedParameters(Utilities::logging_stream::Sptr &log,
                           const params &paramlist) {
  (*log) << "    latent period (theta_l): " << paramlist.T_lat << std::endl;
  (*log) << "    pre-adult probability of symptoms devt (p_s[0]): "
         << paramlist.juvp_s << std::endl;
  (*log) << "    pre-clinical period (theta_i): " << paramlist.T_inf
         << std::endl;
  (*log) << "    asymptomatic period (theta_r): " << paramlist.T_rec
         << std::endl;
  (*log) << "    symptomatic period (theta_s): " << paramlist.T_sym
         << std::endl;
  (*log) << "    hospitalisation stay (theta_h): " << paramlist.T_hos
         << std::endl;
  (*log) << "    bed capacity at hospital (K): " << paramlist.K << std::endl;
  (*log) << "    relative infectiousness of asymptomatic (u): "
         << paramlist.inf_asym << std::endl;
}

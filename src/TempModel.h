...
public:
  using Sptr = std::shared_ptr<TempModel>;
   ...
   * @param log Logger
   */
  TempModel(const CommonModelInputParameters &commonParameters,
   ...
   *
   * @param parameter_set: set of parameters that are being infered (i.e.
   * particles)
   * @param seedlist: seeding method to initialise infection ("random": randomly
   * allocate n infectious individuaals at the start, or "background":
   * background transmission over a defined pre-lockdown period )
   * @param day_shut: day of the lock down
   * @param n_sim_steps: Number of steps to simulate
   *
   * @return Status of model after run
   */
  Status Run(std::vector<double> parameter_set, const seed &seedlist,
             int day_shut, int n_sim_steps) override;
...
private:
   ...
   *    - The first age category (< 20yo) is retained
   *
  ...
  BuildPopulationSeed(const std::vector<int> &age_nums);

  /**
   * @brief Construct the population array
   *
   * Sets up an array for the population at each timestep in each age and
   * disease category also set up the age distribution of old ages as target for
  ...
  BuildPopulationArray(const std::vector<int> &age_nums, const seed &seedlist);

  /**
   * @brief Introduced diseased to the population
   *
   * Compute the total number of susceptible and the number of susceptible per
   * age class
   *
   ...
   */
  static std::vector<double>
  GenerateForcesOfInfection(int &inf_hosp,
                            const std::vector<double> &parameter_set,
                            double u_val, const AgeGroupData &age_data,
                            const std::vector<Compartments> &pops, bool shut);

  /**
   * @brief Generate an infection spread as per the structure of the Working
   * temporary model
   *
   * Creates an infection spread state and counters number of people in
   * different states
   *

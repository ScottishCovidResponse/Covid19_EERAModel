...
public:
     ...
     * @param log Logger
     */
    TempModel(const CommonModelInputParameters& commonParameters, const ObservationsForModels& observations,
        Random::RNGInterface::Sptr rng, Utilities::logging_stream::Sptr log);

    /**
     * @brief Return the model name
     * 
     * @return Returns "irish2"
     */
    std::string ModelName() { return "irish2"; }
...
private:
    ...
    /**
     * @brief Generate an infection spread as per the structure of the Working temporary model
     * 

...
int main(int argc, char **argv) {
  ...
  CommonModelInputParameters commonParameters =
      IO::ReadCommonParameters(params_addr);

  // Import model observational data
  std::string modelConfigDir(std::string(ROOT_DIR) + "/data");
  ObservationsForModels modelObservations =
      IO::ReadModelObservations(modelConfigDir, logger);
  ...
  if (ModelStructureId::ORIGINAL == supplementaryParameters.model_structure) {
    model = std::make_shared<Model::OriginalModel>(
        commonParameters, modelObservations, rng, logger);
  } else if (ModelStructureId::IRISH ==
             supplementaryParameters.model_structure) {
    model = std::make_shared<Model::IrishModel>(commonParameters,
                                                modelObservations, rng, logger);
  } else {
    model = std::make_shared<Model::TempModel>(commonParameters,
                                               modelObservations, rng, logger);
  }
  ...
  if (ModelModeId::PREDICTION == supplementaryParameters.run_type) {
    ...
    PredictionConfig predictionConfig =
        IO::ReadPredictionConfig(configDir, index, logger);
    IO::LogPredictionConfig(predictionConfig, logger);

    // Update the model with the fixed parameters from the prediction
    // configuration
    model->SetFixedParameters(predictionConfig.fixedParameters);

    Prediction::PredictionFramework framework(model, predictionConfig, rng,
                                              out_dir, logger);
    ...
    InferenceConfig inferenceConfig =
        IO::ReadInferenceConfig(configDir, logger);

    IO::LogFixedParameters(commonParameters.paramlist, logger);

    Inference::InferenceFramework framework(model, inferenceConfig, rng,
                                            out_dir, logger);

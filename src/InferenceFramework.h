...
public:
   ...
   *
   * On each loop of the ABC-smc framework, a new collection of particles is
   * accepted by the framework and recorded. This function should be called with
   * the new set of particles, updating its internal weight distribution and
   * kernel windows for the inference parameters
   *
   ...
   * @param inferenceParameterGenerator Parameter generator for th inference
   * framework
   * @param previousParticles Previously accepted inference particles
   */
  particle GenerateNew(int smc, int sim,
                       InferenceParameterGenerator::Sptr parameterGenerator,
   ...
   */
  const std::vector<KernelWindow> KernelWindows() const {
    return kernelWindows_;
  }
...
public:
  ...
  InferenceFramework(Model::ModelInterface::Sptr model,
                     ...
                     Random::RNGInterface::Sptr rng, const std::string &outDir,
...
private:
   ...
   *
   * Runs the model and computes the measures associated with the difference
   * between the model outputs and the observations
   */
  void ModelSelect(EERAModel::particle &outvec, int n_sim_steps, seed seedlist,
                   int day_shut, const std::vector<int> &obsHosp,
   ...
   *   - The sum of squared errors (SSE) between the observed and simulated
   * hospitalisations is less than the configured tolerance limit for the given
   * @p smc step
   *   - The sum of squared errors between the observed and simulated deaths is
   * less than the configured tolerance limit for the given @p smc step
   *
   ...
   *
   * The weight of a given particle is the weighted sum of the weights of all
   * previous particles which are "close" to the given particle.
   *
   ...
   * @param kernelWindows Kernel windows for the inference parameters in the
   * particles
   */
  double ComputeParticleWeight(
   ...
   *
   * Closeness is defined as follows. Two particles are "close" if, for each
   * parameter in their respective parameter sets, the absolute difference
   * between corresponding pairs of particles is less than the corresponding
   * element of the kernel. In other words, if a given particle could have been
   * generated by perturbation of another particle, the two particles are close.
   *
   ...
   *
   * Compute a new set of kernel windows, based on the accepted particles from
   * the last ABC-smc loop. Update the framework's kernel windows with the
 ...
 *
 * Calculates the kernel window (kernel, max limit and min limit) for each
 * inference parameter, based on a set of previously accepted inference
 * particles.
 *
...
ComputeKernelWindow(int nPar, const std::vector<particle> &particleList,
                    double kernelFactor);

/**
 * @brief Compute weight distribution
 *
 * Create the discrete distribution of the weights for the "importance sampling"
 * process
 *
 * @param particleList List of previously accepted particles
 *
...
ComputeWeightDistribution(const std::vector<EERAModel::particle> &particleList);

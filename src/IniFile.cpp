...
void CIniFile::Trim(std::string &str, const std::string &CharsToTrim,
                    int TrimDir) {
...
bool CIniFile::Save(const std::string &FileName,
                    const std::vector<Record> &content) {
  std::ofstream outFile(FileName.c_str()); // Create an output filestream
  if (!outFile.is_open()) {
    return false;
  } // If the output file doesn't open, then return
  ...
  for (const auto &iContent : content) // Loop through each vector
  {
    ...
    if (iContent.Key.empty()) {   // Is this a section?
      outFile << iContent.Commented << "[" << iContent.Section << "]"
              << std::endl; // Then format the section
    } else {
      outFile << iContent.Commented << iContent.Key << "=" << iContent.Value
              << std::endl; // Else format a key/value
    }
  }
...
std::string CIniFile::Content(const std::string &FileName) {
  ...
  if (Load(FileName, content)) // Make sure the file loads
  {
    ...
    {
      if (!iContent.Comments.empty()) {
        s += iContent.Comments;
      } // Add the comments
      if (iContent.Commented != ' ') {
        s += iContent.Commented;
      }                                    // If this is commented, then add it
      if ((iContent.Key.empty())) {        // Is this a section?
        s += '[' + iContent.Section + ']'; // Add the section
      } else {
        s += iContent.Key + '=' + iContent.Value;
      } // Or the Key value to the return srting
...
CIniFile::GetSectionNames(const std::string &FileName) {
  ...
  if (Load(FileName, content)) // Make sure the file is loaded
  {
    ...
    {
      if (iContent.Key
              .empty()) { // If there is no key value, then its a section
...
std::vector<CIniFile::Record>
CIniFile::GetSection(const std::string &SectionName,
  ...
  if (Load(FileName, content)) // Make sure the file is loaded
  {
    ...
    {
      if ((iContent.Section ==
           SectionName) &&         // If this is the section name we want
          (!iContent.Key.empty())) // but not the section name itself
      {
        data.push_back(iContent);
      } // Add the record to the return data
    }
  }
  ...
  if (Load(FileName, content)) // Make sure the file is loaded
  {
    auto iter =
        std::find_if(content.begin(), content.end(),
                     CIniFile::RecordSectionKeyIs(
                         SectionName, KeyName)); // Locate the Section/Key

    if (iter == content.end()) {
      return false;
    } // The Section/Key was not found
  }
...
std::vector<CIniFile::Record> CIniFile::GetRecord(std::string &KeyName,
  ...
  if (Load(FileName, content)) // Make sure the file is loaded
  {
    auto iter = std::find_if(content.begin(), content.end(),
                             CIniFile::RecordSectionKeyIs(
                                 SectionName, KeyName)); // Locate the Record
...
std::string CIniFile::GetValue(std::string &KeyName, std::string &SectionName,
                               const std::string &FileName) {
  if (!EERAModel::Utilities::fileExists(FileName)) {
    throw EERAModel::IO::IOException(FileName + ": File not found!");
  }

  std::vector<Record> content =
      GetRecord(KeyName, SectionName, FileName); // Get the Record
...
bool CIniFile::SetValue(std::string &KeyName, std::string &Value,
                        std::string &SectionName, const std::string &FileName) {
  std::vector<Record> content; // Holds the current record
                               // // Holds the current record
...
bool CIniFile::RenameSection(std::string &OldSectionName,
...
bool CIniFile::CommentRecord(CommentChar &cc, std::string &KeyName,
                             std::string &SectionName,
                             const std::string &FileName) {
  std::vector<Record> content; // Holds the current record
                               // // Holds the current record
...
bool CIniFile::UnCommentRecord(std::string &KeyName, std::string &SectionName,
                               const std::string &FileName) {
  std::vector<Record> content; // Holds the current record
                               // // Holds the current record
...
bool CIniFile::CommentSection(char &CommentChar, std::string &SectionName,
                              const std::string &FileName) {
  std::vector<Record> content; // Holds the current record
                               // // Holds the current record
...
bool CIniFile::UnCommentSection(std::string &SectionName,
                                const std::string &FileName) {
  std::vector<Record> content; // Holds the current record
                               // // Holds the current record
...
bool CIniFile::DeleteRecord(std::string &KeyName, std::string &SectionName,
                            const std::string &FileName) {
  std::vector<Record> content; // Holds the current record
                               // // Holds the current record
...
bool CIniFile::DeleteSection(std::string &SectionName,
                             const std::string &FileName) {
  std::vector<Record> content; // Holds the current record
                               // // Holds the current record
  ...
  if (Load(FileName, content)) // Make sure the file is loaded
  {
    for (int i = static_cast<int>(content.size()) - 1; i > -1;
...
bool CIniFile::SetSectionComments(std::string &Comments,
                                  std::string &SectionName,
                                  const std::string &FileName) {
  std::vector<Record> content; // Holds the current record
                               // // Holds the current record
...
bool CIniFile::SetRecordComments(std::string &Comments, std::string &KeyName,
                                 std::string &SectionName,
                                 const std::string &FileName) {
  std::vector<Record> content; // Holds the current record
                               // // Holds the current record
...
std::vector<CIniFile::Record>
CIniFile::GetSections(const std::string &FileName) {
  std::vector<Record> data;    // Holds the return data
  std::vector<Record> content; // Holds the current record
...
bool CIniFile::AddSection(std::string &SectionName,
                          const std::string &FileName) {
  std::vector<Record> content; // Holds the current record

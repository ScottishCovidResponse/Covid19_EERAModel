...
SimTime GetSimDuration(int time_back, const std::vector<int> &regional_cases) {
  ...
  auto max_time_itr =
      std::find_if(regional_cases.rbegin(), regional_cases.rend(),
                   [](int n) { return n > 0; });
  const int maximum_time = std::distance(max_time_itr, regional_cases.rend());

  // Find the first positive increase in Regional Cases dataset after the start
  // day
  auto first_case_itr =
      std::find_if(regional_cases.begin() + 1, regional_cases.end(),
                   [](int n) { return n > 0; });
  ...
  sim_time_pars.duration = (sim_time_pars.day_intro < 0)
                               ? maximum_time - sim_time_pars.day_intro
                               : maximum_time;

  // Set start time to be zero if negative
  sim_time_pars.day_intro =
      (sim_time_pars.day_intro < 0) ? 0 : sim_time_pars.day_intro;
...
ObsSelect SelectObservations(int &day_shut, const std::vector<int> &timeStamps,
  ...
  // Calculate time under the assumption that time stamps and regional cases
  // data match in size
  ...
  // extra_time is the number of days prior the first detection (index cases)
  // which are considered to show silent spread (spread of disease prior
  // detection). The duration of this period is informed by the parameter hrp.
  // Because we are modelling observed/detected cases and deaths (not true
  // presence), we therefore pad the observation time series with an initial run
  // of zeros.
  if (obs_selections.sim_time.duration > timeSeriesLength) {
    int extra_time = obs_selections.sim_time.duration - timeSeriesLength;
    obs_selections.hospitalised.insert(obs_selections.hospitalised.begin(),
                                       extra_time, 0);
  ...
  (*log) << "    Number of days of obs deaths: " << obs_selections.deaths.size()
         << std::endl;
  (*log) << "    Number of weeks of obs: "
         << static_cast<double>(obs_selections.deaths.size()) / 7.0
         << std::endl;

  // transform  cumulative numbers into incident cases
  std::vector<int> casesIncidence =
      ComputeIncidence(obs_selections.hospitalised);
  obs_selections.hospitalised =
      CorrectIncidence(casesIncidence, obs_selections.hospitalised);
  ...
  obs_selections.deaths =
      CorrectIncidence(deathsIncidence, obs_selections.deaths);
...
std::vector<int> ComputeIncidence(const std::vector<int> &timeseries) {
  ...
  std::adjacent_difference(timeseries.begin(), timeseries.end(),
                           incidence.begin());
...
std::vector<int> CorrectIncidence(const std::vector<int> &originalIncidence,
  ...
  if (std::any_of(originalIncidence.begin(), originalIncidence.end(),
                  [](int i) { return i < 0; })) {
    for (unsigned int ii = 1; ii < originalIncidence.size(); ++ii) {
      if (originalIncidence[ii] < 0) {
        ...
        int case_aft =
            timeseries[ii + 1]; // look at the previous number of cases
        ...
        if (case_bef < case_aft) {
          ...
          // if smaller
          // check when the day prior today was smaller than the next day and
          // remove extra cases to this day
          unsigned int counter_check = 1;
          while (timeseries[ii - counter_check] > case_aft) {
            ++counter_check;
          }

          if (originalIncidence[ii - counter_check + 1] >=
              abs(originalIncidence[ii])) {

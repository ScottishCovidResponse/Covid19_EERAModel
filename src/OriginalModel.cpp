...
OriginalModel::OriginalModel(const CommonModelInputParameters &commonParameters,
  ...
  fixedParameters_ = BuildFixedParameters(observations.waifw_norm.size(),
                                          commonParameters.paramlist);

  ageGroupData_ =
      AgeGroupData{observations.waifw_norm, observations.waifw_home,
                   observations.waifw_sdist, observations.cfr_byage,
                   observations.pf_pop[commonParameters.herd_id - 1]};

  int regionalPopulation =
      GetPopulationOfRegion(observations, commonParameters.herd_id);

  int healthCareWorkers = ComputeNumberOfHCWInRegion(
      regionalPopulation, commonParameters.totN_hcw, observations);

  ageNums_ = ComputeAgeNums(commonParameters.herd_id, regionalPopulation,
                            healthCareWorkers, observations);
...
std::vector<double>
OriginalModel::BuildPopulationSeed(const std::vector<int> &age_nums) {
...
OriginalModel::BuildPopulationArray(const std::vector<int> &age_nums,
                                    const seed &seedlist) {
  ...
  if (seedlist.seedmethod != "background") {
    ...
    startdist[static_cast<int>(rng_->Flat(0, distribution_size))] =
        seedlist.nseed;
...
void OriginalModel::GenerateDiseasedPopulation(
  ...
  int startdz =
      static_cast<int>(rng_->Poisson(static_cast<double>(n_susc) * bkg_lambda));

  // unsigned int startdist[seedarray.size()];
  std::vector<unsigned int> startdist(seedarray.size(), 0);
  rng_->Multinomial(
      seedarray.size(), startdz, &seedarray[0],
      &startdist[0]); // distribute the diseased across the older age categories
...
Status OriginalModel::Run(std::vector<double> parameter_set,
                          const seed &seedlist, int day_shut, int n_sim_steps) {
  Status status = {{0}, {0}, {0}, {}, {}};
  ...
  // Assumes that the number of age groups matches the size of the 'agenums'
  // vector
  std::vector<double> seed_pop = BuildPopulationSeed(ageNums_);

  std::vector<std::vector<double>> parameter_fit(
      ageGroupData_.waifw_norm.size(), parameter_set);
  ...
  for (int tt{1}; tt < n_sim_steps; ++tt) {
    ...
    if (!inLockdown && seedlist.seedmethod == "background") {
      // compute the total number of susceptible and the number of susceptible
      // per age class
      GenerateDiseasedPopulation(poparray, seed_pop,
                                 parameter_set[ModelParameters::LAMBDA]);
    }

    // compute the forces of infection
    std::vector<double> lambda = GenerateForcesOfInfection(
        infection_state.hospitalised, parameter_set,
        fixedParameters_[0].inf_asym, ageGroupData_, poparray, inLockdown);

    // step each agegroup through infections
    for (int age{0}; age < n_agegroup; ++age) {
      InfectionState new_spread = GenerateInfectionSpread(
          poparray[age], infection_state.hospitalised, fixedParameters_[age],
          parameter_fit[age], ageGroupData_.cfr_byage[age], lambda[age]);
...
InfectionState OriginalModel::GenerateInfectionSpread(
    Compartments &pop, const int &n_hospitalised, params fixed_parameters,
  ...
  const int hospitalize =
      static_cast<int>(rng_->Binomial((1.0 - capacity), severe));
  const int nothospitalize = severe - hospitalize;

  const int newdeathsI_s =
      static_cast<int>(rng_->Binomial(p_d * rrd, nothospitalize));
...
std::vector<double> OriginalModel::GenerateForcesOfInfection(
    int &inf_hosp, const std::vector<double> &parameter_set, double u_val,
    const AgeGroupData &age_data, const std::vector<Compartments> &pops,
    bool shut) {
  ...
  double quarantined =
      0.0; // mean proportion reduction in contacts due to quarantine

  std::vector<double> I_mat(n_agegroup);

  // contact matrix
  std::vector<std::vector<double>> waifw =
      (shut) ? std::vector<std::vector<double>>(n_agegroup,
                                                std::vector<double>(n_agegroup))
             : age_data.waifw_norm;

  // age-dependent transmission rate
  std::vector<std::vector<double>> beta(n_agegroup,
                                        std::vector<double>(n_agegroup));

  if (shut) {
    for (int from{0}; from < n_agegroup; ++from) {
      for (int to{0}; to < n_agegroup; ++to) {
        // contact network during shutdown period, assuming a proportion d will
        // properly do it
        waifw[from][to] = (1.0 - d_val) * age_data.waifw_sdist[from][to] +
                          (d_val)*age_data.waifw_home[from][to];
      }
    }
  }
  ...
  // mean proportion reduction in contacts due to quarantine accounting for
  // compliance
  quarantined =
      1 - (quarantined / static_cast<double>(n_agegroup * n_agegroup)) *
              (1 - q_val);
  ...
  for (int from{0}; from < n_agegroup; ++from) {
    if (from <
        (n_agegroup -
         1)) { // n-agegroup-1 to not account for hcw (as different contact)
      // are considered those that are infectious pre-clinical, asymp, tested or
      // symptomatic only asym are infectious pre-clinical and asymptomatic sym
      // are infectious plus those that are tested positive
      int tot_asym = pops[from].I_p +
                     static_cast<int>(u_val * (pops[from].I1 + pops[from].I2 +
                                               pops[from].I3 + pops[from].I4));
      int tot_sym = pops[from].I_t + pops[from].I_s1 + pops[from].I_s2 +
                    pops[from].I_s3 + pops[from].I_s4;

      I_mat[from] = static_cast<double>(tot_asym) +
                    (1 - quarantined) * static_cast<double>(tot_sym);
      // normalisation shouldnt account for dead individuals
      const int tot_pop = accumulate_compartments(pops[from]);
      I_mat[from] =
          static_cast<double>(I_mat[from]) / static_cast<double>(tot_pop);
    }
  ...
  for (int to{0}; to < (n_agegroup); ++to) {
    for (int from{0}; from < (n_agegroup - 1);
         ++from) { // assume perfect self isolation and regular testing of
                   // infected HCW if infected
  ...
  // int rem_pop =
  // pops[n_agegroup-1][4]+pops[n_agegroup-1][5]+pops[n_agegroup-1][6]+pops[n_agegroup-1][8];
  lambda[n_agegroup - 1] =
      p_hcw * c_hcw *
      (static_cast<double>(inf_hosp) / static_cast<double>(tot_pop));

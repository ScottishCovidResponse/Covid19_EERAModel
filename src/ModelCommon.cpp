...
int Flow(Random::RNGInterface::Sptr rng, int pops_from_val,
  ...
  int outs = rng->Poisson(
      rate *
      static_cast<double>(pops_from_val)); // symptomatic become hospitalized
...
std::vector<int> ComputeAgeNums(int shb_id, int Npop, int N_hcw,
  ...
  // define age structure of the shb of interest. the -1 is to account for
  // difference in number of rows between two datasets (age_pop does not have a
  // row with column name)
  const auto &agedist = obs.age_pop[shb_id - 1];

  for (const auto &var : agedist) {
    // modulate the population of non-hcw now to proportion in each age group
    // recorded in 2011
...
int ComputeNumberOfHCWInRegion(int regionalPopulation, int totalHCW,
  ...
  double regionalProportion =
      static_cast<double>(regionalPopulation) / scotlandPopulation;
...
compartments_to_vector(const std::vector<Compartments> &cmps_vec) {
  ...
  for (auto cmps : cmps_vec) {
    _temp.push_back({cmps.S, cmps.E, cmps.E_t, cmps.I_p, cmps.I_t, cmps.I1,
                     cmps.I2, cmps.I3, cmps.I4, cmps.I_s1, cmps.I_s2, cmps.I_s3,

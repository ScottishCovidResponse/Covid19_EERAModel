...
std::vector<double>
IrishModel::BuildPopulationSeed(const std::vector<int> &age_nums) {
...
IrishModel::BuildPopulationArray(const std::vector<int> &age_nums,
                                 const seed &seedlist) {
...
Status IrishModel::Run(std::vector<double> parameter_set, const seed &seedlist,
...
InfectionState IrishModel::GenerateInfectionSpread(
...
std::vector<double> IrishModel::GenerateForcesOfInfection(
  ...
  for (int from{0}; from < n_agegroup; ++from) {
    if (from <
      ...
      int tot_asym = static_cast<int>(pops[from].I_p +
                                      u_val * (pops[from].I1 + pops[from].I2 +

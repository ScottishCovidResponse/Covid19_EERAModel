...
void read_parameters(int& herd_id, double& tau, int&num_threads,int& nsteps, int& nParticLimit, int& nSim, double& kernelFactor, 
		...
		double& prior_rrd_shape1,double& prior_rrd_shape2,double& prior_rrr_shape1,double& prior_rrr_shape2,
...
int main(int argc, char **argv) {
	...
	double prior_rrd_shape1,prior_rrd_shape2,prior_rrr_shape1,prior_rrr_shape2;
	...
	read_parameters(herd_id, tau, num_threads, nsteps, nParticLimit,nSim,kernelFactor,toleranceLimit,
			...
			prior_rrd_shape1,prior_rrd_shape2,prior_rrr_shape1,prior_rrr_shape2,
	...
	flag1 = {prior_pinf_shape1, prior_phcw_shape1, prior_chcw_mean, prior_d_shape1, prior_q_shape1, 
		prior_rrd_shape1,prior_rrr_shape1,prior_rrh_shape1,prior_lambda_shape1};	
	flag2 = { prior_pinf_shape2, prior_phcw_shape2, prior_chcw_mean, prior_d_shape2, prior_q_shape2,
		prior_rrd_shape2,prior_rrr_shape2,prior_rrh_shape2,prior_lambda_shape2};			
	...
	for (smc = 0; smc < nsteps; ++smc) {//todo:
		...
		output_step << "iterID,nsse_cases,nsse_deaths,p_inf,p_hcw,c_hcw,d,q,rrd,rrr,rrh,intro,weight\n";
...
void infspread(gsl_rng * r, vector<int>& pop, int& deaths, int& deathsH, int& detected, 
	...
	double rrd = parameter_set[5];
	double rrr = parameter_set[6];
	double rrh = parameter_set[7];	
		
    // hospitalized
    unsigned int newdeathsH = gsl_ran_poisson(r, ( pf_val * rrd + (1 - pf_val) ) * p_d * (1 / T_hos) * (double)H);
	newdeathsH = min(H,newdeathsH);
    D += newdeathsH;
	H -= newdeathsH;
    unsigned int recoverH = gsl_ran_poisson(r, ( pf_val * rrr + (1 - pf_val) ) * (1 - p_d) * (1 / T_hos) * (double)H);//hospitalized recover
	recoverH=min(H,recoverH) ;
    R += recoverH;
	H -= recoverH;
	
    // symptomatic
    unsigned int hospitalize = gsl_ran_poisson(r, ( pf_val * rrh + (1 - pf_val) ) * p_h * (1 / T_sym) * (double)I_s); //symptomatic become hospitalized
    hospitalize = min(I_s, hospitalize);
	H += hospitalize;
	I_s -= hospitalize;
	
    unsigned int newdeathsC = gsl_ran_poisson(r, ( pf_val * rrd + (1 - pf_val) ) * ( 1 - p_h ) * p_dc * ( 2 / T_hos) * (double)I_s); //symptomatic die at home / in communities	
    newdeathsC = min(I_s, newdeathsC);
    D += newdeathsC;
	I_s -= newdeathsC;
	
    unsigned int recoverI_s = gsl_ran_poisson(r, ( pf_val * rrr + (1 - pf_val) ) * ( 1 - p_h ) * (1 - p_dc) * ( 1 / T_rec) * (double)I_s); //symptomatic recover at home / in communities	
...
void read_parameters(int& herd_id, double& tau, int&num_threads,int& nsteps, int& nParticLimit, int& nSim, double& kernelFactor, 
	...
	prior_rrd_shape1= atof(parameters.GetValue("prior_rrd_shape1", "Priors settings", parameterfile).c_str());
	prior_rrd_shape2=atof(parameters.GetValue("prior_rrd_shape2", "Priors settings", parameterfile).c_str());
	prior_rrr_shape1=atof(parameters.GetValue("prior_rrr_shape1", "Priors settings", parameterfile).c_str());
	prior_rrr_shape2=atof(parameters.GetValue("prior_rrr_shape2", "Priors settings", parameterfile).c_str());

...
void read_parameters(int& herd_id, double& tau, int&num_threads,int& nsteps, int& nParticLimit, int& nSim, double& kernelFactor, vector<double>& toleranceLimit,
		params& paramlist, int& day_shut, int& totN_hcw,
...
int main(int argc, char **argv) {
	...
	select_obs(	Npop, t_index, duration, day_intro, day_shut, 
		obsHosp_tmp, data_tmp, herd_id, paramlist.T_inf, paramlist.T_sym);
	obsHosp = obsHosp_tmp;
	obsHosp_tmp.clear();	
...
//				outs_vec.sum_sq = 1.e06;
				...
				model_select(smc,outs_vec, fixed_parameters, cfr_byage,pf_byage,waifw_norm,waifw_sdist,waifw_home,
							agenums,tau, duration, day_intro, day_shut, Npop, r);
...
//	outvec.sum_sq = sum_sq;
	...
	for (unsigned int ii = 0; ii < sim_status.size(); ++ii) {
		...
		outvec.death_outs.push_back(death_status[ii]);
...
void select_obs(int& Npop, int& t_index, int& duration, int& day_intro, int& day_shut, 
	...
	cout << "Obs cum data: " << obsHosp_tmp.size()<<'\n';
...
void my_model(vector<double> parameter_set, vector<params> fixed_parameters, vector<vector<double>> cfr_byage, 
	...
	unsigned int startdz = dfseed; //how many diseased to start with
//	size_t k = sizeof(seed_pop);
//	unsigned int startdist[k];
	vector<int> startdist = {0,0,0,0};
	startdist[(int)gsl_ran_flat(r, 0, 4)] = startdz;

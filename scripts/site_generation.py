        ...
        with open(input_file) as f:
            for line in f:
                try:
                    ...
                    self._clang_tidy_data[file_name][address] = {'message' : message, 
        ...
        for line in f:
          if re.findall('(^src/\w+[.h|.cpp].+)', line):
            if _lines_str:
              ...
              self._flawfinder_data[_file][_address] = {'message' : _message, 'library' : _library, 'file' : _file}
            _lines_str = line
    ...
    {body}
      ...
      for entry in self._flawfinder_data:
        ...
        for address in sorted(self._flawfinder_data[entry].keys()):
                ...
                '''.format(file=entry, address=address,
...
'''
        ...
        for entry in self._clang_tidy_data:
            ...
            for address in sorted(self._clang_tidy_data[entry].keys()):
                is_warning = 'warning' in self._clang_tidy_data[entry][address]['message'].lower()
